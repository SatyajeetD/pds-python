...
def check_nvme_modules_and_load():
    ...
    p1.stdout.close()
    returncode=p2.wait()
    load_constants=pds_host_operations.pds_get_constants()
    #If returncode is non-zero, it means the modules are not loaded ,load the modules
    if returncode != 0:
        #load_constants=pds_host_operations.pds_get_constants()
        load_nvme_core=subprocess.Popen(['modprobe','nvme_core', 'io_timeout='+str(load_constants['io_timeout'])],stdout=subprocess.PIPE)
        load_nvme_fabrics=subprocess.Popen(['modprobe','nvme_fabrics'],stdout=subprocess.PIPE)
        load_nvme_protocol=subprocess.Popen(['modprobe','nvme_'+str(load_constants['protocol'])],stdout=subprocess.PIPE)
        if load_nvme_core.wait()==0 and load_nvme_fabrics.wait()==0 and load_nvme_protocol.wait()==0:
        ...
        logger.info("Checking IO timeout parameter")
        cmd="/sys/module/nvme_core/parameters/io_timeout"
        io=subprocess.Popen(["cat",cmd],stdout=subprocess.PIPE)
        timeout=io.communicate()[0]
        print timeout
        if timeout.strip()=='600':
            logger.info("Timeout already set to 600 no changes needed")
        else:
            logger.info("Timeout not 600 attempting reload of the nvme modules")
            rm_module=subprocess.Popen(["rmmod","nvme_"+str(load_constants["protocol"]),"nvme_fabrics","nvme_core"])
            if rm_module.wait()==0:
                logger.info("Module unload successful, reloading with timeout set to 600")
                load_nvme_core=subprocess.Popen(['modprobe','nvme_core', 'io_timeout='+str(load_constants['io_timeout'])],stdout=subprocess.PIPE)
                load_nvme_fabrics=subprocess.Popen(['modprobe','nvme_fabrics'],stdout=subprocess.PIPE)
                load_nvme_protocol=subprocess.Popen(['modprobe','nvme_'+str(load_constants['protocol'])],stdout=subprocess.PIPE)
                if load_nvme_core.wait()==0 and load_nvme_fabrics.wait()==0 and load_nvme_protocol.wait()==0:
                ...
                lod_module=subprocess.Popen(["modprobe","nvme_"+str(load_constants["protocol"])])
                logger.info("Unable to unload module, continuing with timeout set to "+str(timeout.strip()))
