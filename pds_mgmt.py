...
clusterops.add_argument('-n','--add_nqns', nargs='+',required=False,help='Connect specific NQNs')
...
configops.add_argument('-d','--delete_ips', nargs='+',required=False,help='Delete IPs from CLI is needed')
configops.add_argument('-c','--clustername',required=False,help='Add IPs from CLI is needed')
configops.add_argument('-n','--add_nqns', nargs='+',required=False,help='Add NQNs from CLI is needed')
...
if args.optype.lower()=='cluster':
    ...
    iplist=args.ip
    if args.add_nqns:
        nqnlist=args.add_nqns
    else:
        nqnlist=[]
    #if op.lower()=='connect':
    if iplist and len(clusterlist) > 1:
        print("Operation Failed, multiple clusters provided with -i")
        exit()
    ...
    if args.connect:
        ...
        for cluster in clusterlist:
            ...
            result=pds_host_operations.cluster_connect(cluster,nqnlist,iplist)
            if result == 0:
                print("Successfully connected to cluster "+str(cluster))
    #elif op.lower()=='disconnect':
    elif args.disconnect:
        if clusterlist and nqnlist and not iplist:
            print("Missing input for IP to be disconnected")
            exit()
        if clusterlist and not nqnlist and not iplist:
            print("Issuing cluster disconnect request for cluster(s) " + str(" ".join(args.clusterlist)))
        if clusterlist and iplist and not nqnlist:
            if len(args.clusterlist) != 1:
                print("Multi cluster operation not supported with IP input")
                exit()
         
        logger.info('Disconnect request for '+str(clusterlist))
        for cluster in clusterlist:
            pds_host_operations.cluster_disconnect(cluster,nqnlist,iplist)
    #elif op.lower()=='get':
...
if args.optype.lower()=='configure':
    ...
    if action == 2:
        ...
        if args.filename:
        ...
        elif not args.filename and not args.add_ips and not args.clustername and not args.delete_ips:
            update_file(pds_constants.pds_config_file,loglevel)
            print("Successfully updated PDS loglevel to " + str(loglevel))
        elif args.add_ips and args.clustername and not args.add_nqns:
            #user_response=raw_input("Confirm if " + str(" ".join(args.add_ips)) + "should be added to " + str(args.clustername) + "(Y/N). \nRun command with -n/--add_nqns to add IP-NQN maps\n")
            ...
            exit()
        elif args.clustername and not args.add_ips and not args.delete_ips:
            print("Both IP and cluster name inputs expected while updating Cluster definition only one input provided")
            exit()
        elif (args.delete_ips and not args.clustername) or (args.clustername and not args.delete_ips and not args.add_ips and not args.add_nqns):
            print("Both IP and cluster name inputs expected while updating Cluster definition only one input provided here") 
            exit()
        elif args.add_ips and args.add_nqns and args.clustername:
            if len(args.add_ips) > 1:
                print("Only 1 IP permitted for adding NQN maps")
                exit()
            dict_to_be_added_to_pds_conf={}
            dict_to_be_added_to_pds_conf[args.add_ips[0]]=[]
            
            for nqn in args.add_nqns:
                if nqn not in dict_to_be_added_to_pds_conf.values():
                    dict_to_be_added_to_pds_conf[args.add_ips[0]].append(nqn)
            #print (dict_to_be_added_to_pds_conf)
            ip_nqn_updated_config=pds_host_operations.add_nqn_map_to_config(dict_to_be_added_to_pds_conf, args.clustername.capitalize())
            #print ip_nqn_updated_config
            print loglevel
            update_file(pds_constants.pds_config_file,loglevel,ip_nqn_updated_config)
            #pds_host_operations.add_nqns_to_pds_configuration()
        elif args.clustername and args.delete_ips and not args.add_nqns:
            clustername = args.clustername.capitalize()
            #Add validation for iplist
            pds_host_operations.delete_ips_from_cluster(clustername,args.delete_ips)        
    
        elif args.clustername and args.delete_ips and args.add_nqns:
            if len(args.delete_ips) > 1:
                print ("NQN removal with multiple IPs not supported")
                exit()
            nqnlist=args.add_nqns
            pds_host_operations.delete_ips_from_cluster(args.clustername, args.delete_ips, nqnlist )
        else:
